...
def addorders():
    if request.method=='POST':
        ...
        cn.execute(f"insert into orders(product_name,customer_id,quantity) values('{product_name}','{customer_id}','{quantity}')")
        conn.commit()
        print('Data has been inserted')
        return jsonify({'message':'sucessfull'})
    else:
...
def deletecustomer():
    if request.method=='POST':
        cn=conn.cursor()
        customer_name = request.form.get("name")
        cn.execute(f"delete from customer where customer_name='{customer_name}')")
        conn.commit()
        print('Data has been inserted')
        return jsonify({'message':'sucessfull'})
    else:
